...
class FileParser:
    ...
    def parse_file(self):
        ...
        lp_variables = self._get_lp_variables(lp_problem[:-1])
        constraint_matrix = self._setup_constraint_matrix(lp_problem[1:-1], lp_variables)
        is_maximization = self._check_maximization(lp_problem[0])
    ...
    def _setup_constraint_matrix(self, constraints: list, variables: list) -> np.ndarray:
    ...
    def _get_lp_variables(self, lp_problem: list) -> list:
        global_vars = []
    ...
    def _check_maximization(self, objective_function: str) -> bool:
